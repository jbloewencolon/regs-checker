...
- **NVIDIA API key** (`NVIDIA_API_KEY` in `.env`) for the primary provider — or **LM Studio** on localhost:1234 with `google/gemma-4-26b-a4b` loaded for the local fallback (`"provider": "local"` in `config/agent_models.json`)
