...
def validate_extraction_against_orrick(
    ...
    SFH-1f (audit SF-08): also returns None when the Orrick text was
    LLM-GENERATED by `--mode enrich-orrick` (``orrick_source ==
    'llm_generated'``, stamped by orrick_enrichment.py at write time). A
    generated summary entering this scorer means 50% of the confidence score
    becomes an LLM agreeing with an LLM on the same law text — correlated
    errors scoring as validation. Generated summaries remain usable as
    extraction context and triage keyword seeds (those paths don't run
    through this function); they are only quarantined from *scoring*.
    Operator-approved consequence (2026-07-06): enrich-orrick-only laws
    drop to the tracker-absent gated path until the SFH-3 confidence
    re-architecture gives tracker-silent laws a fair path.
    """
    if context.get("orrick_source") == "llm_generated":
        return None
...
# SFH-1f: canonical tracker-metadata keys on DocumentFamily.metadata_. The
# 'enforcement' vs 'enforcement_penalties' episode ("silently wrong for
# months") was a WRITER-side key rename that the reader experienced as
# "no Orrick data" — no error anywhere. Absence can't be validated (tracker-
# silent laws legitimately have no keys), but a tracker-ish key OUTSIDE this
# set is exactly the rename-drift signature and should be shouted about.
TRACKER_METADATA_KNOWN_KEYS = frozenset({
    "bill_id",
    "orrick_summary",
    "key_requirements",
...
_TRACKER_KEY_STEMS = ("orrick", "requirement", "enforcement", "scope", "iapp")


def find_suspicious_tracker_keys(metadata: dict | None) -> list[str]:
    """Return metadata keys that look tracker-related but aren't canonical.

    e.g. a writer producing 'enforcement' instead of 'enforcement_penalties'
    shows up here instead of silently reading as tracker-absent downstream.
    Non-tracker keys (whatever else lives in metadata_) are ignored.
    """
    if not metadata:
        return []
    return sorted(
        key
        for key in metadata
        if key not in TRACKER_METADATA_KNOWN_KEYS
        and any(stem in key.lower() for stem in _TRACKER_KEY_STEMS)
    )
